...
# class AccountPasswordChangeTest(APITestCase):
#     def setUp(self):
#         self.client = APIClient()

#         self.pwd_hash = AuthenticationService.get_password_hash("password", "1")
#         self.account = Account.objects.create(login="admin@test.com", pwd_hash=self.pwd_hash, unique_token="1")
...
#     @pytest.mark.django_db
#     def test_update_another_acccount(self):
#         account_data = {
#             'login': 'test@example.com',
#             'password': 'password',
#         }
...
#                 'new_password': 'new'
#             })
#         account = Account.objects.filter(id=self.account.id).first()
#         assert account.pwd_hash == AuthenticationService.get_password_hash('new', self.token.account.unique_token)
#         assert response.status_code == status.HTTP_200_OK
