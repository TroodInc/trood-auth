...
def test_creates_and_returns_intermediate_token_on(client: Client):
    ...
    assert_that(decoded_response['data']['token'], has_length(6))
    assert_that(IntermediateToken.objects.get(token=decoded_response['data']['token']), instance_of(IntermediateToken))
...
def test_returns_error_with_invalid_phone_number(client: Client):
    ...
    assert_that(decoded_response['data']['phone'][0], equal_to('Phone has invalid length'))
...
def test_returns_error_with_duplicated_phone_number(client: Client):
    ...
    SecondAuthFactorFactory(account=account, factor_id=phone_number, type=SECOND_AUTH_FACTOR_TYPE.PHONE)
    data = {
        'login': account.login,
        'phone': phone_number
    }
    ...
    assert_that(response.status_code, equal_to(status.HTTP_400_BAD_REQUEST))
    assert_that(decoded_response['data']['phone'][0], equal_to('Account with that phone already exists'))
